...
public class Chaining {
    ...
    public int chain (int key) {
        ...
        int hash = ((this.A * key) % (power2(this.w))) >> (w-r); 
        return hash;
    }

    /**helper method that checks if slot n is empty **/ 
    ...
    public int insertKey(int key){
        ...
        if(isEmpty(index) == true) {
            this.Table.get(index).ensureCapacity(1); 
            this.Table.get(index).add(0, key); 
            return 0; }
            else {
        for( int i = 0; i< this.m; i++) {
            int size = Table.get(index).size(); 
            this.Table.get(index).ensureCapacity(size + 1); 
